...
//tag used for console messages
static const char TAG[] = "wifi_app";

//Queue used to manipulate the main queue of events
...
//Initializes the default tcp stack and default wifi config.
...
//configures the wifi ap settings and assigns the static IP to the Soft AP
static void wifi_app_soft_ap_config(void)
{
	//Soft AP-wifi ap configuration
	...
	ESP_ERROR_CHECK(esp_wifi_set_bandwidth(WIFI_IF_AP, WIFI_AP_BANDWIDTH)); //20Mghz
...
static void wifi_app_task(void *pvParameters)
{
	...
	wifi_app_event_handler_init(); //initialise event handler
	wifi_app_default_wifi_init(); //initialize TCP/IP stack and wifi config
	wifi_app_soft_ap_config(); // Soft AP config

	//Start wifi
	ESP_ERROR_CHECK(esp_wifi_start());

	//send first event message
	wifi_app_send_mesage(WIFI_APP_MSG_START_HTTP_SERVER);
